...
tools/run.sh    # boots the VM headless, runs the investigation, checks evidence (~1 min)
```
...
Not a folder of sample files — **`agent-starter/` after one investigation**.
Same four directories, filled in, so you can read the finished thing against
the empty one you start from:

```
worked-example/                          agent-starter/  (yours)
├── tools/                               ├── tools/       vm, ksrc, report-check
│   ├── run.sh        boot + drive        │
│   ├── investigate.sh   guest side       │   (your own scripts land here too)
│   └── check.sh      the 16 asserts      │
├── triggers/cow-trigger.c               ├── triggers/    empty, grows weekly
├── probes/probe-do-wp-page.bt           ├── probes/      empty, grows weekly
└── reports/vm-cow-01/  ← one hand-in    └── reports/batch-NN/<question-id>/
    ├── report.md      prose, batches 1–3    ├── report.md (or report.json)
    ├── report.json    structured, batch 4+  ├── repro.sh
    ├── repro.sh                             └── evidence/
    └── evidence/
```

Two differences, both deliberate. This workspace's `tools/` holds only what
is specific to this investigation — the course tool layer (`vm`, `ksrc`,
`report-check`) it borrows from `agent-starter/` next door rather than
shipping a second copy to keep in sync. And its report sits at
`reports/vm-cow-01/` with no `batch-NN/` level, because this question came
from no batch; yours always has one
...
so you can read the same investigation in both renderings.
...
`tools/report-check` expects to find them next to a report. And note where
the instruments are *not*: the trigger and probe live in `triggers/` and
`probes/`, because they are reused across weeks and graded as a library,
while a report directory holds the record of one investigation.
...
**3. Trigger design** ([`triggers/cow-trigger.c`](triggers/cow-trigger.c)). One
...
**4. Instrumentation** ([`probes/probe-do-wp-page.bt`](probes/probe-do-wp-page.bt)).
...
**5. Run + evidence** (private mapping, from [`reports/vm-cow-01/evidence/`](reports/vm-cow-01/evidence/)):
...
[`reports/vm-cow-01/report.md`](reports/vm-cow-01/report.md) in prose and
[`reports/vm-cow-01/report.json`](reports/vm-cow-01/report.json) structured. Either way every
...
**The hand-in** — `reports/vm-cow-01/`, the part you copy the shape of:

| File | Role |
|---|---|
| `reports/vm-cow-01/report.md` | the answer in prose — the batch 1–3 hand-in format |
| `reports/vm-cow-01/report.json` | the same answer structured (design-doc §7 schema) — the batch 4+ format |
| `reports/vm-cow-01/repro.sh` | **the model for the `repro.sh` you hand in every week** — re-run against an already-booted VM and assert; scratch output by default, `REPRO_ARCHIVE=1` to refresh `evidence/` |
| `reports/vm-cow-01/evidence/` | raw captures from a real passing run — the model for the `evidence/` directory in your own hand-in |

**The instrument libraries** — outside the hand-in, because they outlive it:

| File | Role |
|---|---|
| `triggers/cow-trigger.c` | deterministic trigger workload (private + shared modes) |
| `probes/probe-do-wp-page.bt` | bpftrace probe on the wp-fault path |

**This workspace's tools** — the example's own harness; your `tools/` carries
the course layer (`vm`, `ksrc`, `report-check`) plus whatever you add:

| File | Role |
|---|---|
| `tools/run.sh` | host side: boot VM headless, run the investigation via the autorun channel, check evidence |
| `tools/investigate.sh` | guest side: what `run.sh` executes inside the VM (compile → attach → run → collect) |
| `tools/check.sh` | the 16 machine-checkable assertions (replay-gate prototype) |
